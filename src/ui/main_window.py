...
from ..ui.sensitivity_panel import SensitivityAnalysisPanel
from ..ui.risk_benefit_plot import RiskBenefitPlot
...
# ---------------------------------------------------------------------------
# Patient view — full scrollable, trace, sensitivity panel, risk‑benefit plot
# ---------------------------------------------------------------------------
class PatientView(QWidget):
    ...
    def _build(self) -> None:
        ...
        # Ranked actions table (4 columns – no Risk Badge)
    ...
    def load_patient(self, patient: Patient, actions: list[Action]) -> None:
        ...
        self.risk_benefit_plot.update_for_patient(patient.patient_id)   # <-- update plot
        self._refresh()
        ...
        for from_state in states:
            for to_state in states:
                ...
                if abs(p_after - p_before) > 1e-9:
                    ...
                    row_text = f"{from_state} → {to_state}:   {p_before:.3f}  →  {p_after:.3f}  {arrow}"
                    lbl = _label(row_text)
                    lbl.setStyleSheet(f"color: {color}; background: transparent; border: none; font-size: 13px;")
                    self._transition_layout.addWidget(lbl)

        if not changed:
            self._transition_layout.addWidget(_label("No transition probabilities changed.", muted=True))
...
class Sidebar(QWidget):
    ...
    def _build(self) -> None:
        ...
        nav_items = ["Dashboard", "Patients", "Patient Management", "Analytics", "Trends"]
...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        self._stack.addWidget(self._patient_view)     # 1
        self._stack.addWidget(self._management_view)  # 2
        self._stack.addWidget(self._analytics_view)   # 3
        self._stack.addWidget(self._trend_view)       # 4
